...
askPassword() {
	# password=1234
	local password=$(cloudPassword.sh backup)
	echo $password
}
...
encPipe=$(
	enc() {
		local cipher=$1
		echo "gpg --batch --compress-algo none -c --cipher-algo $cipher '--passphrase=${password}_$cipher' 2> /dev/null"
	};
...
decPipe=$(
	dec() {
		local cipher=$1
		echo "gpg -d '--passphrase=${password}_$cipher' 2> /dev/null"
	}
	ret=""; for algo in ${algos[@]}; do
		wrap=$(dec $algo)
		ret="$wrap | $ret"
